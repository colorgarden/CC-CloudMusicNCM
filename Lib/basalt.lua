...
project["log.lua"] = function(...) local ca={}ca._logs={}ca._enabled=false;ca._logToFile=false
ca._logFile="basalt.log"fs.delete(ca._logFile)
ca.LEVEL={DEBUG=1,INFO=2,WARN=3,ERROR=4}
local da={[ca.LEVEL.DEBUG]="Debug",[ca.LEVEL.INFO]="Info",[ca.LEVEL.WARN]="Warn",[ca.LEVEL.ERROR]="Error"}
local _b={[ca.LEVEL.DEBUG]=colors.lightGray,[ca.LEVEL.INFO]=colors.white,[ca.LEVEL.WARN]=colors.yellow,[ca.LEVEL.ERROR]=colors.red}local function ab(_c)return
type(_c)=="table"and _c.type=="bimg"and _c.bimg~=nil end
local function bb(_c)if
ab(_c)then local ac=tostring(_c.text or"")
return{type="bimg",text=ac,bimg=_c.bimg,message=ac}end;return
{type="text",text=tostring(_c),message=tostring(_c)}end;function ca.setLogToFile(_c)ca._logToFile=_c end
function ca.setEnabled(_c)ca._enabled=_c end;local function cb(_c)
if ca._logToFile then local ac=io.open(ca._logFile,"a")if ac then
ac:write(_c.."\n")ac:close()end end end
local function db(_c,...)if
not ca._enabled then return end;local ac=os.date("%H:%M:%S")
local bc=debug.getinfo(3,"Sl")local cc=bc.source:match("@?(.*)")local dc=bc.currentline
local _d=string.format("[%s:%d]",cc:match("([^/\\]+)%.lua$"),dc)local ad="["..da[_c].."]"local bd={}local cd=""for __a,a_a in
ipairs(table.pack(...))do local b_a=bb(a_a)bd[#bd+1]=b_a;if __a>1 then cd=cd.." "end
cd=cd..b_a.message end
local dd=string.format("%s %s%s %s",ac,_d,ad,cd)cb(dd)
table.insert(ca._logs,{time=ac,level=_c,message=cd,parts=bd})end;function ca.debug(...)db(ca.LEVEL.DEBUG,...)end;function ca.info(...)
db(ca.LEVEL.INFO,...)end
function ca.warn(...)db(ca.LEVEL.WARN,...)end;function ca.error(...)db(ca.LEVEL.ERROR,...)end;return ca end
...
local da=require("libraries/expect")local _b=require("errorManager")local ab={}ab.__index=ab
ab._properties={}local bb={}
local function cb(_c)if type(_c)=="string"then return _c end;return tostring(_c)end;ab._setterHooks={}function ab.addSetterHook(_c)
table.insert(ab._setterHooks,_c)end
local function db(_c,ac,bc,cc)for dc,_d in ipairs(ab._setterHooks)do
local ad=_d(_c,ac,bc,cc)if ad~=nil then bc=ad end end;return bc end
function ab.defineProperty(_c,ac,bc)
if not rawget(_c,'_properties')then _c._properties={}end
_c._properties[ac]={type=bc.type,default=bc.default,canTriggerRender=bc.canTriggerRender,getter=bc.getter,setter=bc.setter,allowNil=bc.allowNil}local cc=ac:sub(1,1):upper()..ac:sub(2)
_c[
...
type(_d)~="function"then
if bc.type=="table"then if _d==nil then
if not bc.allowNil then da(2,_d,bc.type)end end else da(2,_d,bc.type)end end;if
bc.setter then _d=bc.setter(dc,_d,...)end
dc:_updateProperty(ac,_d)return dc end
_c["get"..cc.."State"]=function(dc,_d,...)da(1,dc,"element")return
dc.getPropertyState(ac,_d,...)end
_c["set"..cc.."State"]=function(dc,_d,ad,...)da(1,dc,"element")
dc.setPropertyState(ac,_d,ad,...)return dc end end
function ab.combineProperties(_c,ac,...)local bc={...}for dc,_d in pairs(bc)do
if not _c._properties[_d]then _b.error("Property not found: "..
cb(_d))end end;local cc=
ac:sub(1,1):upper()..ac:sub(2)
_c["get"..cc]=function(dc)
da(1,dc,"element")local _d={}
for ad,bd in pairs(bc)do table.insert(_d,dc.get(bd))end;return table.unpack(_d)end
_c["set"..cc]=function(dc,...)da(1,dc,"element")local _d={...}for ad,bd in pairs(bc)do
dc.set(bd,_d[ad])end;return dc end end
function ab.blueprint(_c,ac,bc,cc)
if not bb[_c]then
...
ad=getmetatable(ad)and rawget(getmetatable(ad),'__index')end;bb[_c]=_d end;local dc={_values={},_events={},loadedCallback=function()end}
dc.get=function(_d)
...
return bb[_c][ad]end})return dc end
function ab.createFromBlueprint(_c,ac,bc)local cc=_c.new({},bc)
for dc,_d in pairs(ac._values)do if type(_d)=="table"then
cc._values[dc]=ca(_d)else cc._values[dc]=_d end end;return cc end
function ab:__init()self._values={}self._observers={}self._states={}
self._modifiedProperties={}
self.set=function(dc,_d,...)local ad=self._values[dc]local bd=self._properties[dc]
if
(bd~=nil)then if(bd.setter)then _d=bd.setter(self,_d,...)end;if bd.canTriggerRender then
self:updateRender()end;self._values[dc]=db(self,dc,_d,bd)
self._modifiedProperties[dc]=true
if ad~=_d and self._observers[dc]then for cd,dd in
ipairs(self._observers[dc])do dd(self,_d,ad)end end end end
self.get=function(dc,...)local _d=self._values[dc]local ad=self._properties[dc]if
(ad==nil)then
_b.error("Property not found: "..cb(dc))return end;if type(_d)=="function"and
ad.type~="function"then _d=_d(self)end;return ad.getter and
ad.getter(self,_d,...)or _d end
self.setPropertyState=function(dc,_d,ad,...)local bd=self._properties[dc]
if(bd~=nil)then if(bd.setter)then
ad=bd.setter(self,ad,...)end;ad=db(self,dc,ad,bd)if
not self._states[_d]then self._states[_d]={}end
self._states[_d][dc]=ad;local cd=self._values.currentState
if cd==_d then if bd.canTriggerRender then
self:updateRender()end
if self._observers[dc]then for dd,__a in
ipairs(self._observers[dc])do __a(self,ad,nil)end end end end end
self.getPropertyState=function(dc,_d,...)local ad=self._states and self._states[_d]and
self._states[_d][dc]local bd=
ad~=nil and ad or self._values[dc]
local cd=self._properties[dc]if(cd==nil)then
_b.error("Property not found: "..cb(dc))return end;if type(bd)=="function"and
cd.type~="function"then bd=bd(self)end;return cd.getter and
cd.getter(self,bd,...)or bd end
self.getResolved=function(dc,...)local _d=self:getActiveStates()local ad=nil;for cd,dd in ipairs(_d)do
if
self._states and
self._states[dd.name]and self._states[dd.name][dc]~=nil then ad=self._states[dd.name][dc]break end end;if ad==
nil then ad=self._values[dc]end
local bd=self._properties[dc]if(bd==nil)then
_b.error("Property not found: "..cb(dc))return end;if type(ad)=="function"and
bd.type~="function"then ad=ad(self)end;return bd.getter and
bd.getter(self,ad,...)or ad end;local _c={}local ac=getmetatable(self).__index
while ac do if
...
function ab:_updateProperty(_c,ac)local bc=self._values[_c]
if type(bc)=="function"then bc=bc(self)end;self._modifiedProperties[_c]=true;self._values[_c]=ac
local cc=
type(ac)=="function"and ac(self)or ac
if bc~=cc then
...
_d(self,cc,bc)end end end;return self end
function ab:observe(_c,ac)
self._observers[_c]=self._observers[_c]or{}table.insert(self._observers[_c],ac)return self end
function ab:removeObserver(_c,ac)
if self._observers[_c]then
for bc,cc in ipairs(self._observers[_c])do if cc==ac then
table.remove(self._observers[_c],bc)
if#self._observers[_c]==0 then self._observers[_c]=nil end;break end end end;return self end;function ab:removeAllObservers(_c)
if _c then self._observers[_c]=nil else self._observers={}end;return self end
function ab:instanceProperty(_c,ac)
ab.defineProperty(self,_c,ac)self._values[_c]=ac.default;return self end
function ab:removeProperty(_c)self._values[_c]=nil;self._properties[_c]=nil;self._observers[_c]=
nil
local ac=_c:sub(1,1):upper().._c:sub(2)self["get"..ac]=nil;self["set"..ac]=nil;self["get"..ac.."State"]=
nil;self["set"..ac.."State"]=nil;return self end
function ab:getPropertyConfig(_c)return self._properties[_c]end;return ab end
...
ca.defineProperty(ca,"text",{default="Button",type="string",canTriggerRender=true})
ca.defineProperty(ca,"content",{default=nil,type="table",allowNil=true,canTriggerRender=true})ca.defineEvent(ca,"mouse_click")
...
ca._setText=ca.setText
function ca:setText(da)self.set("content",nil)return ca._setText(self,da)end
function ca:setContent(da)self.set("content",da)return self end
function ca:getContent()return self.getResolved("content")end;function ca:setImage(da)return self:setContent(da)end
function ca:render()
aa.render(self)local da=self.getResolved("text")
local _b=self.getResolved("width")local ab=self.getResolved("height")
local bb=self.getResolved("content")local cb,db,_c=self:getContentSize(bb,da)if _c=="text"then
da=da:sub(1,_b)local ac,bc=ba(da,_b,ab)
self:textFg(ac,bc,da,self.getResolved("foreground"))return end
if
cb>0 and db>0 then
local ac=math.max(1,math.floor((_b-cb)/2)+1)
local bc=math.max(1,math.floor((ab-db)/2)+1)self:drawContent(ac,bc,_b-ac+1,ab-bc+1,bb)end end;return ca end
...
d.defineProperty(d,"content",{default=nil,type="table",allowNil=true,canTriggerRender=true,setter=function(_a,aa)local ba=_a.getResolved("checkedContent")
local ca=select(1,_a:getContentSize(aa))local da=select(2,_a:getContentSize(aa))
local _b=select(1,_a:getContentSize(ba,_a.getResolved("checkedText")))
local ab=select(2,_a:getContentSize(ba,_a.getResolved("checkedText")))ca=math.max(ca,_b)da=math.max(da,ab,1)
if
(_a.getResolved("autoSize"))then _a.set("width",ca)_a.set("height",da)end;return aa end})
...
d.defineProperty(d,"checkedContent",{default=nil,type="table",allowNil=true,canTriggerRender=true,setter=function(_a,aa)local ba=_a.getResolved("content")
local ca=select(1,_a:getContentSize(ba,_a.getResolved("text")))
local da=select(2,_a:getContentSize(ba,_a.getResolved("text")))local _b=select(1,_a:getContentSize(aa))
local ab=select(2,_a:getContentSize(aa))ca=math.max(ca,_b)da=math.max(da,ab,1)
if
(_a.getResolved("autoSize"))then _a.set("width",ca)_a.set("height",da)end;return aa end})
...
c.init(self,_a,aa)self.set("type","CheckBox")end;d._setText=d.setText;function d:setText(_a)self.set("content",nil)return
d._setText(self,_a)end;function d:setContent(_a)
self.set("content",_a)return self end;function d:getContent()
return self.getResolved("content")end
function d:setImage(_a)return self:setContent(_a)end;d._setCheckedText=d.setCheckedText
function d:setCheckedText(_a)
self.set("checkedContent",nil)return d._setCheckedText(self,_a)end
function d:setCheckedContent(_a)self.set("checkedContent",_a)return self end
function d:getCheckedContent()return self.getResolved("checkedContent")end
function d:setCheckedImage(_a)return self:setCheckedContent(_a)end
...
local ca=_a and
self.getResolved("checkedContent")or self.getResolved("content")if ca~=nil then
self:drawContent(1,1,self.getResolved("width"),self.getResolved("height"),ca)return end
local da=string.sub(_a and ba or aa,1,self.getResolved("width"))
self:textFg(1,1,da,self.getResolved("foreground"))end;return d end
...
local function _b(ab,bb,cb)return math.max(bb,math.min(cb,ab))end
da.defineProperty(da,"dropdownHeight",{default=5,type="number"})
da.defineProperty(da,"selectedText",{default="",type="string"})
da.defineProperty(da,"selectedContent",{default=nil,type="table",allowNil=true,canTriggerRender=true})
da.defineProperty(da,"dropSymbol",{default="\31",type="string"})
da.defineProperty(da,"undropSymbol",{default="\17",type="string"})function da.new()local ab=setmetatable({},da):__init()
ab.class=da;ab.set("width",16)ab.set("height",1)ab.set("z",8)
return ab end
function da:init(ab,bb)
ba.init(self,ab,bb)self.set("type","DropDown")
self:registerState("opened",nil,200)
self:observe("items",function()self:syncHeight()end)
self:observe("dropdownHeight",function()self:syncHeight()end)return self end;da._setSelectedText=da.setSelectedText;function da:setSelectedText(ab)
self.set("selectedContent",nil)local bb=da._setSelectedText(self,ab)self:syncHeight()
return bb end
function da:setSelectedContent(ab)
self.set("selectedContent",ab)self:syncHeight()return self end
function da:getSelectedContent()return self.getResolved("selectedContent")end
function da:setSelectedImage(ab)return self:setSelectedContent(ab)end
function da:getCurrentDisplayContent()local ab=self:getSelectedItems()
if#ab>0 then local bb=ab[1]return
bb.content or bb.image,bb.text or""end
return self.getResolved("selectedContent"),self.getResolved("selectedText")end
function da:getCollapsedHeight()local ab,bb=self:getCurrentDisplayContent()
local cb,db=self:getContentSize(ab,bb)return math.max(1,db)end
function da:getExpandedListHeight()return
math.min(self.getResolved("dropdownHeight"),self:getTotalContentHeight())end
function da:getListViewportHeight()return self:getExpandedListHeight()end
function da:getScrollViewportHeight()return self:getExpandedListHeight()end
function da:getListContentWidth()local ab=self.getResolved("width")
local bb=self:getListViewportHeight()local cb=self.getResolved("showScrollBar")and
self:getTotalContentHeight()>bb;return cb and
(ab-1)or ab,cb end
function da:getListRelativePosition(ab,bb)local cb,db=self:getRelativePosition(ab,bb)
local _c=self:getCollapsedHeight()return cb,db-_c end
function da:syncHeight()local ab=self:getCollapsedHeight()
local bb=self:hasState("opened")and
self:getExpandedListHeight()or 0;self.set("height",ab+bb)return self end
function da:mouse_click(ab,bb,cb)
if not aa.mouse_click(self,ab,bb,cb)then return false end;local db,_c=self:getRelativePosition(bb,cb)
local ac=self:hasState("opened")local bc=self:getCollapsedHeight()
if _c<=bc then if ac then
self:unsetState("opened")else self:setState("opened")end
self:syncHeight()return true elseif ac and _c>bc then
local cc,dc=self:getListRelativePosition(bb,cb)local _d=self.getResolved("width")
local ad=self:getListViewportHeight()local bd=self.getResolved("showScrollBar")
local cd=self:getTotalContentHeight()
if bd and cd>ad and cc==_d then local dd=cd-ad
local __a=math.max(1,math.floor((ad/cd)*ad))local a_a=
dd>0 and(self.getResolved("offset")/dd*100)or 0;local b_a=
math.floor((a_a/100)* (ad-__a))+1
if dc>=b_a and dc<b_a+__a then
self._scrollBarDragging=true;self._scrollBarDragOffset=dc-b_a else local c_a=math.max(1,ad-__a)local d_a=(
(dc-1)/c_a)*100
local _aa=math.floor((d_a/100)*dd+0.5)self.set("offset",_b(_aa,0,dd))end end;return true end;return false end
function da:mouse_drag(ab,bb,cb)
if self:hasState("opened")and self._scrollBarDragging then
local db,_c=self:getListRelativePosition(bb,cb)local ac=self:getListViewportHeight()
local bc=math.max(1,self:getTotalContentHeight())local cc=math.max(1,math.floor((ac/bc)*ac))local dc=math.max(0,
bc-ac)
_c=math.max(1,math.min(ac,_c))local _d=_c- (self._scrollBarDragOffset or 0)local ad=math.max(1,
ac-cc)local bd=( (_d-1)/ad)*100;local cd=math.floor(
(bd/100)*dc+0.5)
self.set("offset",_b(cd,0,dc))return true end;return
aa.mouse_drag and aa.mouse_drag(self,ab,bb,cb)or false end
function da:mouse_up(ab,bb,cb)
if self:hasState("opened")then
if self._scrollBarDragging then self._scrollBarDragging=false;self._scrollBarDragOffset=
nil;self:unsetState("clicked")return true end;local db,_c=self:getRelativePosition(bb,cb)
local ac=self:getCollapsedHeight()
if _c>ac and self.getResolved("selectable")then
local bc=self.getResolved("items")local cc,dc=self:getListRelativePosition(bb,cb)
local _d,ad=self:getListContentWidth()if ad and cc==self.getResolved("width")then
self:unsetState("clicked")return true end
local bd,cd=self:getItemAtRow(dc,self.getResolved("offset"),_d)
if bd~=nil and cd then
if type(cd)=="string"then cd={text=cd}bc[bd]=cd end;if not self.getResolved("multiSelection")then
for dd,__a in ipairs(bc)do if
type(__a)=="table"then __a.selected=false end end end;cd.selected=not
cd.selected;if cd.callback then cd.callback(self)end
self:fireEvent("select",bd,cd)self:unsetState("opened")
self:unsetState("clicked")self:syncHeight()self:updateRender()return true end end;self:unsetState("clicked")return true end;return
aa.mouse_up and aa.mouse_up(self,ab,bb,cb)or false end
function da:mouse_scroll(ab,bb,cb)
if not aa.mouse_scroll(self,ab,bb,cb)then return false end
if self:hasState("opened")then
local db,_c=self:getRelativePosition(bb,cb)local ac=self:getCollapsedHeight()
if _c>ac then
local bc=self.getResolved("offset")
local cc=math.max(0,self:getTotalContentHeight()-self:getListViewportHeight())bc=math.min(cc,math.max(0,bc+ab))
self.set("offset",bc)return true end end;return true end;function da:key(ab)local bb=ba.key(self,ab)if bb then self:syncHeight()end
return bb end
function da:render()aa.render(self)
local ab=self.getResolved("width")local bb=self.getResolved("height")
local cb,db=self:getCurrentDisplayContent()local _c=self:hasState("opened")
local ac=self:getCollapsedHeight()local bc=self.getResolved("background")
local cc=self.getResolved("foreground")
if _c then local _d=bb;local ad=self:getExpandedListHeight()
self.set("height",ad)ba.render(self,ac)self.set("height",_d)end
local dc=_c and self.getResolved("dropSymbol")or
self.getResolved("undropSymbol")for row=1,ac do
self:blit(1,row,string.rep(" ",ab-1).. (row==1 and dc or" "),string.rep(ca[cc],ab),string.rep(ca[bc],ab))end
if cb~=nil then
self:drawContent(1,1,
ab-1,ac,cb,{foreground=cc,background=bc})else db=db:sub(1,ab-1)self:textFg(1,1,db,cc)end end;function da:focus()aa.focus(self)self:prioritize()
self:setState("opened")self:syncHeight()end;function da:blur()
aa.blur(self)self:unsetState("opened")self:syncHeight()
self:updateRender()end;return da end
...
project["elements/Image.lua"] = function(...) local ca=require("elementManager")
local da=ca.getElement("VisualElement")local _b=require("libraries/colorHex")
local ab=setmetatable({},da)ab.__index=ab;local function bb(_c,ac,bc,cc)
if type(_c)~="string"or#_c==0 then return _c end;_c=_c:gsub(" ",cc)_c=_c:gsub("Q",ac)_c=_c:gsub("B",bc)
return _c end
ab.defineProperty(ab,"bimg",{default={{}},type="table",canTriggerRender=true})
ab.defineProperty(ab,"currentFrame",{default=1,type="number",canTriggerRender=true})
ab.defineProperty(ab,"autoResize",{default=false,type="boolean"})
ab.defineProperty(ab,"offsetX",{default=0,type="number",canTriggerRender=true})
ab.defineProperty(ab,"offsetY",{default=0,type="number",canTriggerRender=true})
ab.combineProperties(ab,"offset","offsetX","offsetY")
function ab.new()local _c=setmetatable({},ab):__init()
_c.class=ab;_c.set("width",12)_c.set("height",6)
_c.set("background",colors.black)_c.set("z",5)return _c end;function ab:init(_c,ac)da.init(self,_c,ac)self.set("type","Image")
return self end
function ab:resizeImage(_c,ac)
local bc=self.getResolved("bimg")
for cc,dc in ipairs(bc)do local _d={}
for y=1,ac do local ad=string.rep(" ",_c)
local bd=string.rep("f",_c)local cd=string.rep("0",_c)
if dc[y]and dc[y][1]then local dd=dc[y][1]
local __a=dc[y][2]local a_a=dc[y][3]
ad=(dd..string.rep(" ",_c)):sub(1,_c)
bd=(__a..string.rep("f",_c)):sub(1,_c)
cd=(a_a..string.rep("0",_c)):sub(1,_c)end;_d[y]={ad,bd,cd}end;bc[cc]=_d end;self:updateRender()return self end
function ab:getImageSize()local _c=self.getResolved("bimg")if
not _c[1]or not _c[1][1]then return 0,0 end;return#_c[1][1][1],#_c[1]end
function ab:getPixelData(_c,ac)
local bc=self.getResolved("bimg")[self.getResolved("currentFrame")]if not bc or not bc[ac]then return end;local cc=bc[ac][1]
local dc=bc[ac][2]local _d=bc[ac][3]
if not cc or not dc or not _d then return end;local ad=tonumber(dc:sub(_c,_c),16)
local bd=tonumber(_d:sub(_c,_c),16)local cd=cc:sub(_c,_c)return ad,bd,cd end
local function cb(_c,ac)
local bc=_c.getResolved("bimg")[_c.getResolved("currentFrame")]if not bc then bc={}
_c.getResolved("bimg")[_c.getResolved("currentFrame")]=bc end
if not bc[ac]then bc[ac]={"","",""}end;return bc end
local function db(_c,ac,bc)if not _c.getResolved("autoResize")then return end
local cc=_c.getResolved("bimg")local dc=ac;local _d=bc
for ad,bd in ipairs(cc)do for cd,dd in pairs(bd)do dc=math.max(dc,#dd[1])
_d=math.max(_d,cd)end end
for ad,bd in ipairs(cc)do
for y=1,_d do if not bd[y]then bd[y]={"","",""}end;local cd=bd[y]while#cd[1]<
dc do cd[1]=cd[1].." "end;while#cd[2]<dc do
cd[2]=cd[2].."f"end;while#cd[3]<dc do cd[3]=cd[3].."0"end end end end
function ab:setText(_c,ac,bc)if
type(bc)~="string"or#bc<1 or _c<1 or ac<1 then return self end
if
not self.getResolved("autoResize")then local _d,ad=self:getImageSize()if ac>ad then return self end end;local cc=cb(self,ac)if self.getResolved("autoResize")then
db(self,_c+#bc-1,ac)else local _d=#cc[ac][1]if _c>_d then return self end
bc=bc:sub(1,_d-_c+1)end
local dc=cc[ac][1]
cc[ac][1]=dc:sub(1,_c-1)..bc..dc:sub(_c+#bc)self:updateRender()return self end
function ab:getText(_c,ac,bc)if not _c or not ac then return""end
local cc=self.getResolved("bimg")[self.getResolved("currentFrame")]if not cc or not cc[ac]then return""end;local dc=cc[ac][1]if not dc then
return""end
if bc then return dc:sub(_c,_c+bc-1)else return dc:sub(_c,_c)end end
function ab:setFg(_c,ac,bc)if
type(bc)~="string"or#bc<1 or _c<1 or ac<1 then return self end
if
not self.getResolved("autoResize")then local _d,ad=self:getImageSize()if ac>ad then return self end end;local cc=cb(self,ac)if self.getResolved("autoResize")then
db(self,_c+#bc-1,ac)else local _d=#cc[ac][2]if _c>_d then return self end
bc=bc:sub(1,_d-_c+1)end
local dc=cc[ac][2]
cc[ac][2]=dc:sub(1,_c-1)..bc..dc:sub(_c+#bc)self:updateRender()return self end
function ab:getFg(_c,ac,bc)if not _c or not ac then return""end
local cc=self.getResolved("bimg")[self.getResolved("currentFrame")]if not cc or not cc[ac]then return""end;local dc=cc[ac][2]if not dc then
return""end
if bc then return dc:sub(_c,_c+bc-1)else return dc:sub(_c)end end
function ab:setBg(_c,ac,bc)if
type(bc)~="string"or#bc<1 or _c<1 or ac<1 then return self end
if
not self.getResolved("autoResize")then local _d,ad=self:getImageSize()if ac>ad then return self end end;local cc=cb(self,ac)if self.getResolved("autoResize")then
db(self,_c+#bc-1,ac)else local _d=#cc[ac][3]if _c>_d then return self end
bc=bc:sub(1,_d-_c+1)end
local dc=cc[ac][3]
cc[ac][3]=dc:sub(1,_c-1)..bc..dc:sub(_c+#bc)self:updateRender()return self end
function ab:getBg(_c,ac,bc)if not _c or not ac then return""end
local cc=self.getResolved("bimg")[self.getResolved("currentFrame")]if not cc or not cc[ac]then return""end;local dc=cc[ac][3]if not dc then
return""end
if bc then return dc:sub(_c,_c+bc-1)else return dc:sub(_c)end end
function ab:setPixel(_c,ac,bc,cc,dc)if bc then self:setText(_c,ac,bc)end;if cc then
self:setFg(_c,ac,cc)end;if dc then self:setBg(_c,ac,dc)end;return self end
function ab:applyPalette()
local _c=self.getResolved("bimg")[self.getResolved("currentFrame")]if not _c then return self end;if not _c.palette then return self end
local ac=self:getBaseFrame():getTerm()self.oldPalette={}local bc=_c.palette
for cc,dc in pairs(bc)do
if type(dc)=="table"then
local ad,bd,cd=dc[1],dc[2],dc[3]
ac.setPaletteColor(2 ^cc,colors.packRGB(ad,bd,cd))else ac.setPaletteColor(2 ^cc,dc)end;local _d=ac.getPaletteColor(cc)self.oldPalette[cc]=_d end;self:updateRender()return self end
function ab:undoPalette()if not self.oldPalette then return self end
local _c=self:getBaseFrame():getTerm()
for ac,bc in pairs(self.oldPalette)do _c.setPaletteColor(2 ^ac,bc)end;self.oldPalette=nil;self:updateRender()return self end
function ab:nextFrame()if not self.getResolved("bimg").animation then
return self end;local _c=self.getResolved("bimg")
local ac=self.getResolved("currentFrame")local bc=ac+1;if bc>#_c then bc=1 end;self.set("currentFrame",bc)
return self end
function ab:addFrame()local _c=self.getResolved("bimg")
local ac=_c.width or#_c[1][1][1]local bc=_c.height or#_c[1]local cc={}local dc=string.rep(" ",ac)
local _d=string.rep("f",ac)local ad=string.rep("0",ac)for y=1,bc do cc[y]={dc,_d,ad}end
table.insert(_c,cc)return self end;function ab:updateFrame(_c,ac)local bc=self.getResolved("bimg")bc[_c]=ac
self:updateRender()return self end;function ab:getFrame(_c)
local ac=self.getResolved("bimg")
return ac[_c or self.getResolved("currentFrame")]end
function ab:getMetadata()local _c={}
local ac=self.getResolved("bimg")
for bc,cc in pairs(ac)do if(type(cc)=="string")then _c[bc]=cc end end;return _c end
function ab:setMetadata(_c,ac)if(type(_c)=="table")then
for cc,dc in pairs(_c)do self:setMetadata(cc,dc)end;return self end
local bc=self.getResolved("bimg")if(type(ac)=="string")then bc[_c]=ac end;return self end
function ab:render()da.render(self)
local _c=self.getResolved("bimg")[self.getResolved("currentFrame")]if not _c then return end;local ac=self.getResolved("offsetX")
local bc=self.getResolved("offsetY")local cc=self.getResolved("width")
local dc=self.getResolved("height")local _d=_b[self.getResolved("foreground")]
local ad=_b[self.getResolved("background")]
for y=1,dc do local bd=y+bc;local cd=_c[bd]
if cd then local dd=cd[1]local __a=bb(cd[2],_d,ad,_d)
local a_a=bb(cd[3],_d,ad,ad)
if dd and __a and a_a then local b_a=cc-math.max(0,ac)
if b_a>0 then if ac<0 then local c_a=
math.abs(ac)+1;dd=dd:sub(c_a)__a=__a:sub(c_a)
a_a=a_a:sub(c_a)end;dd=dd:sub(1,b_a)
__a=__a:sub(1,b_a)a_a=a_a:sub(1,b_a)
self:blit(math.max(1,1 +ac),y,dd,__a,a_a)end end end end end;return ab end
...
ca.defineProperty(ca,"content",{default=nil,type="table",allowNil=true,canTriggerRender=true,setter=function(da,_b)local ab=da.getResolved("autoSize")
local bb,cb=da:getContentSize(_b)
if ab then da.set("width",bb)
da.set("height",math.max(1,cb))else da.set("height",math.max(1,cb))end;return _b end})
...
return self end;ca._setText=ca.setText
function ca:setText(da)self.set("content",
nil)return ca._setText(self,da)end
function ca:setContent(da)self.set("content",da)return self end
function ca:getContent()return self.getResolved("content")end;function ca:setImage(da)return self:setContent(da)end
...
local _b=self.getResolved("content")if _b~=nil then
self:drawContent(1,1,self.getResolved("width"),self.getResolved("height"),_b)return end
if
(self.getResolved("autoSize"))then
self:textFg(1,1,da,self.getResolved("foreground"))else local ab=ba(da,self.getResolved("width"))for bb,cb in ipairs(ab)do
self:textFg(1,bb,cb,self.getResolved("foreground"))end end end;return ca end
...
project["elements/List.lua"] = function(...) local aa=require("elements/Collection")
local ba=require("libraries/colorHex")local ca=setmetatable({},aa)ca.__index=ca
ca.defineProperty(ca,"offset",{default=0,type="number",canTriggerRender=true,setter=function(ab,bb)
local cb=math.max(0,
ab:getTotalContentHeight()-ab:getScrollViewportHeight())return math.min(cb,math.max(0,bb))end})
ca.defineProperty(ca,"emptyText",{default="No items",type="string",canTriggerRender=true})
...
ca.defineProperty(ca,"scrollBarBackground",{default="\127",type="string",canTriggerRender=true})
ca.defineProperty(ca,"scrollBarColor",{default=colors.lightGray,type="color",canTriggerRender=true})
ca.defineProperty(ca,"scrollBarBackgroundColor",{default=colors.gray,type="color",canTriggerRender=true})ca.defineEvent(ca,"mouse_click")
ca.defineEvent(ca,"mouse_up")ca.defineEvent(ca,"mouse_drag")
ca.defineEvent(ca,"mouse_scroll")ca.defineEvent(ca,"key")
local da={text={type="string",default="Entry"},content={type="table",default=nil},image={type="table",default=
nil},bg={type="number",default=nil},fg={type="number",default=nil},selectedBg={type="number",default=nil},selectedFg={type="number",default=
nil},callback={type="function",default=nil}}
local function _b(ab,bb,cb)return math.max(bb,math.min(cb,ab))end;function ca.new()local ab=setmetatable({},ca):__init()
ab.class=ca;ab.set("width",16)ab.set("height",8)ab.set("z",5)
return ab end
function ca:init(ab,bb)
aa.init(self,ab,bb)self._entrySchema=da;self.set("type","List")
self:observe("items",function()
local cb=math.max(0,
self:getTotalContentHeight()-self:getScrollViewportHeight())
if self.getResolved("offset")>cb then self.set("offset",cb)end end)
self:observe("height",function()
local cb=math.max(0,self:getTotalContentHeight()-
self:getScrollViewportHeight())
if self.getResolved("offset")>cb then self.set("offset",cb)end end)return self end
function ca:_normalizeItem(ab,bb)if type(ab)=="string"then ab={text=ab}if bb then
self.getResolved("items")[bb]=ab end end;return ab end
function ca:getScrollViewportHeight()return self.getResolved("height")end
function ca:_getContentWidth()local ab=self.getResolved("width")
local bb=
self.getResolved("showScrollBar")and
self:getTotalContentHeight(ab)>self:getScrollViewportHeight()return bb and(ab-1)or ab,bb end
function ca:getItemHeight(ab,bb)ab=self:_normalizeItem(ab)if not ab then return 0 end;if
ab.separator then return 1 end
local cb=math.max(1,bb or self.getResolved("width"))local db=ab.content or ab.image;if db~=nil then
local _c,ac=self:getContentSize(db,ab.text or"")return math.max(1,ac)end;return 1 end
function ca:getTotalContentHeight(ab)local bb=self.getResolved("items")or{}
local cb=math.max(1,
ab or self.getResolved("width"))local db=0
for _c,ac in ipairs(bb)do db=db+
self:getItemHeight(self:_normalizeItem(ac,_c),cb)end;return db end
function ca:getVisibleItems(ab,bb,cb)local db=self.getResolved("items")or{}
local _c={}local ac=bb or 0;local bc=1
local cc=math.max(1,cb or self.getResolved("width"))
for dc,_d in ipairs(db)do local ad=self:_normalizeItem(_d,dc)
local bd=self:getItemHeight(ad,cc)
if ac>=bd then ac=ac-bd else local cd=ac;local dd=math.min(bd-cd,ab-bc+1)
if dd>0 then
table.insert(_c,{index=dc,item=ad,y=bc,sourceY=cd,visibleHeight=dd,itemHeight=bd})bc=bc+dd;if bc>ab then break end end;ac=0 end end;return _c end
function ca:getItemAtRow(ab,bb,cb)
for db,_c in
ipairs(self:getVisibleItems(self:getScrollViewportHeight(),bb or
self.getResolved("offset"),cb))do if ab>=_c.y and ab<_c.y+_c.visibleHeight then
return _c.index,_c.item,_c end end;return nil,nil,nil end
...
local bc=self.getResolved("items")local cc=self:getScrollViewportHeight()
local dc=self.getResolved("showScrollBar")local _d=self:getTotalContentHeight()
if
dc and _d>cc and db==ac then local ad=_d-cc
local bd=math.max(1,math.floor((cc/_d)*cc))local cd=
ad>0 and(self.getResolved("offset")/ad*100)or 0;local dd=
math.floor((cd/100)* (cc-bd))+1
if _c>=dd and _c<dd+bd then
self._scrollBarDragging=true;self._scrollBarDragOffset=_c-dd else local __a=math.max(1,cc-bd)local a_a=
( (_c-1)/__a)*100
local b_a=math.floor((a_a/100)*ad+0.5)self.set("offset",_b(b_a,0,ad))end;return true end
if self.getResolved("selectable")then local ad=self:_getContentWidth()
local bd,cd=self:getItemAtRow(_c,self.getResolved("offset"),ad)
if bd~=nil and cd then if not self.getResolved("multiSelection")then
for dd,__a in
ipairs(bc)do if type(__a)=="table"then __a.selected=false end end end;cd.selected=not
cd.selected;if cd.callback then cd.callback(self)end
self:fireEvent("select",bd,cd)self:updateRender()end end;return true end;return false end
function ca:mouse_drag(ab,bb,cb)
if self._scrollBarDragging then local db,_c=self:getRelativePosition(bb,cb)
local ac=self:getScrollViewportHeight()
local bc=math.max(1,self:getTotalContentHeight())local cc=math.max(1,math.floor((ac/bc)*ac))local dc=math.max(0,
bc-ac)
_c=math.max(1,math.min(ac,_c))local _d=_c- (self._scrollBarDragOffset or 0)local ad=math.max(1,
ac-cc)local bd=( (_d-1)/ad)*100;local cd=math.floor(
...
self.set("offset",_b(cd,0,dc))return true end;return
aa.mouse_drag and aa.mouse_drag(self,ab,bb,cb)or false end
function ca:mouse_up(ab,bb,cb)if self._scrollBarDragging then self._scrollBarDragging=false
self._scrollBarDragOffset=nil;return true end
return aa.mouse_up and
aa.mouse_up(self,ab,bb,cb)or false end
function ca:mouse_scroll(ab,bb,cb)
if aa.mouse_scroll(self,ab,bb,cb)then
local db=self.getResolved("offset")
local _c=math.max(0,self:getTotalContentHeight()-self:getScrollViewportHeight())db=math.min(_c,math.max(0,db+ab))
self.set("offset",db)return true end;return false end
function ca:onSelect(ab)self:registerCallback("select",ab)return self end
function ca:scrollToBottom()
local ab=math.max(0,self:getTotalContentHeight()-
self:getScrollViewportHeight())self.set("offset",ab)return self end
function ca:scrollToTop()self.set("offset",0)return self end
function ca:scrollToItem(ab)local bb=self:getScrollViewportHeight()
local cb=self.getResolved("offset")local db=self:_getContentWidth()local _c=0;local ac=0;for bc,cc in
ipairs(self.getResolved("items"))do local dc=self:getItemHeight(cc,db)
if bc==ab then ac=_c+dc;break end;_c=_c+dc end
if
_c<cb then self.set("offset",math.max(0,_c))elseif ac>cb+bb then self.set("offset",math.max(0,
ac-bb))end;return self end
function ca:key(ab)
if aa.key(self,ab)and self.getResolved("selectable")then
local bb=self.getResolved("items")local cb=self:getSelectedIndex()
if ab==keys.up then
self:selectPrevious()if cb and cb>1 then self:scrollToItem(cb-1)end
return true elseif ab==keys.down then self:selectNext()if cb and cb<#bb then
self:scrollToItem(cb+1)end;return true elseif ab==keys.home then
self:clearItemSelection()self:selectItem(1)self:scrollToTop()return true elseif
ab==keys["end"]then self:clearItemSelection()self:selectItem(#bb)
self:scrollToBottom()return true elseif ab==keys.pageUp then local db=self:getScrollViewportHeight()local _c=math.max(1,(
cb or 1)-db)
self:clearItemSelection()self:selectItem(_c)self:scrollToItem(_c)return true elseif ab==
keys.pageDown then local db=self:getScrollViewportHeight()
local _c=math.min(#bb,(cb or 1)+db)self:clearItemSelection()self:selectItem(_c)
self:scrollToItem(_c)return true end end;return false end
function ca:render(ab)ab=ab or 0;aa.render(self)
local bb=self.getResolved("items")local cb=self:getScrollViewportHeight()
local db=self.getResolved("offset")local _c=self.getResolved("width")
local ac=self.getResolved("background")local bc=self.getResolved("foreground")
local cc=self.getResolved("showScrollBar")local dc=self:getTotalContentHeight()local _d=cc and dc>cb;local ad=
_d and _c-1 or _c
if#bb==0 then
local bd=self.getResolved("emptyText")local cd=math.floor(cb/2)+ab;local dd=math.max(1,
math.floor((_c-#bd)/2)+1)for i=1,cb do
self:textBg(1,i,string.rep(" ",_c),ac)end;if cd>=1 and cd<=cb then
self:textFg(dd,cd+ab,bd,colors.gray)end;return end
for i=1,cb do self:textBg(1,i+ab,string.rep(" ",ad),ac)end
for bd,cd in ipairs(self:getVisibleItems(cb,db,ad))do local dd=cd.item
local __a=cd.y+ab
if dd.separator then
local a_a=( (dd.text or"-")~=""and dd.text or"-"):sub(1,1)local b_a=string.rep(a_a,ad)local c_a=dd.fg or bc;local d_a=dd.bg or ac
for line=0,
cd.visibleHeight-1 do
self:textBg(1,__a+line,string.rep(" ",ad),d_a)self:textFg(1,__a+line,b_a,c_a)end else local a_a=dd.text or""local b_a=dd.content or dd.image
local c_a=dd.selected
local d_a=c_a and
(dd.selectedBg or self.getResolved("selectedBackground"))or(dd.bg or ac)
local _aa=c_a and
(dd.selectedFg or self.getResolved("selectedForeground"))or(dd.fg or bc)for line=0,cd.visibleHeight-1 do
self:textBg(1,__a+line,string.rep(" ",ad),d_a)end
if b_a~=nil then
self:drawContent(1,__a,ad,cd.visibleHeight,b_a,{sourceY=cd.sourceY,foreground=_aa,background=d_a})else local aaa=a_a
if#aaa>ad then aaa=ad>3 and(aaa:sub(1,ad-3).."...")or
aaa:sub(1,ad)else aaa=aaa..
string.rep(" ",ad-#aaa)end;self:textFg(1,__a,aaa,_aa)end end end
if _d then
local bd=math.max(1,math.floor((cb/dc)*cb))local cd=dc-cb;local dd=cd>0 and(db/cd*100)or 0;local __a=math.floor(
(dd/100)* (cb-bd))+1
local a_a=self.getResolved("scrollBarSymbol")local b_a=self.getResolved("scrollBarBackground")
local c_a=self.getResolved("scrollBarColor")local d_a=self.getResolved("scrollBarBackgroundColor")
for i=1,cb do self:blit(_c,
i+ab,b_a,ba[bc],ba[d_a])end;for i=__a,math.min(cb,__a+bd-1)do
self:blit(_c,i+ab,a_a,ba[c_a],ba[d_a])end end end;return ca end
...
local _b={text={type="string",default="Entry"},content={type="table",default=nil},image={type="table",default=nil},bg={type="number",default=nil},fg={type="number",default=
nil},selectedBg={type="number",default=nil},selectedFg={type="number",default=nil},callback={type="function",default=nil},dropdown={type="table",default=
nil}}function da.new()local ab=setmetatable({},da):__init()
...
self.set("width",self:getTotalWidth(),true)end
self.set("height",self:getMenuHeight(),true)end)
self.set("height",self:getMenuHeight(),true)return self end
function da:getTotalWidth()local ab=self.getResolved("items")
local bb=self.getResolved("spacing")local cb=0
for db,_c in ipairs(ab)do
if type(_c)=="table"then
cb=cb+select(1,self:getContentSize(_c.content or _c.image,
_c.text or""))else cb=cb+#tostring(_c)+2 end;if db<#ab then cb=cb+bb end end;return cb end
function da:getItemHeight(ab)
if type(ab)=="string"then ab={text=" "..ab.." "}end
local bb,cb=self:getContentSize(ab.content or ab.image,ab.text or"")return math.max(1,cb)end
function da:getDropdownItemHeight(ab,bb)if type(ab)=="string"then ab={text=ab}end;if
(
ab.text=="---"or ab.label=="---")and not(ab.content or ab.image)then return 1 end
local cb,db=self:getContentSize(ab.content or ab.image,
ab.text or ab.label or"")return math.max(1,db)end
function da:getMenuHeight()local ab=self.getResolved("items")or{}local bb=1
for cb,db in
ipairs(ab)do bb=math.max(bb,self:getItemHeight(db))end;return bb end
function da:render()aa.render(self)local ab=self.getResolved("width")
local bb=self.getResolved("height")local cb=self.getResolved("spacing")
local db=self.getResolved("horizontalOffset")local _c=self.getResolved("items")local ac={}local bc=1
for dc,_d in ipairs(_c)do
if
type(_d)=="string"then _d={text=" ".._d.." "}_c[dc]=_d end
local ad=select(1,self:getContentSize(_d.content or _d.image,_d.text or""))
ac[dc]={startX=bc,endX=bc+ad-1,text=_d.text,content=_d.content or _d.image,width=ad,height=self:getItemHeight(_d),item=_d}bc=bc+ad;if dc<#_c and cb>0 then bc=bc+cb end end
for dc,_d in ipairs(ac)do local ad=_d.item;local bd=_d.startX-db;local cd=_d.endX-db
if bd>ab then break end
if cd>=1 then local dd=math.max(1,bd)local __a=math.min(ab,cd)
local a_a=math.max(1,1 -bd+1)local b_a=math.min(_d.width,_d.width- (cd-ab))local c_a=
__a-dd+1
if c_a>0 then local d_a=ad.selected
local _aa=ad.selectable==false and
self.getResolved("separatorColor")or
(d_a and(ad.selectedForeground or
self.getResolved("selectedForeground"))or(
ad.foreground or self.getResolved("foreground")))
local aaa=d_a and
(ad.selectedBackground or self.getResolved("selectedBackground"))or
(ad.background or self.getResolved("background"))local baa=math.min(bb,_d.height)for row=1,baa do
self:blit(dd,row,string.rep(" ",c_a),string.rep(ca[_aa],c_a),string.rep(ca[aaa],c_a))end
if _d.content~=nil then
self:drawContent(dd,1,c_a,baa,_d.content,{sourceX=
a_a-1,foreground=_aa,background=aaa})else local caa=_d.text:sub(a_a,b_a)
self:blit(dd,1,caa,string.rep(ca[_aa],#caa),string.rep(ca[aaa],
#caa))end end
if dc<#_c and cb>0 then local d_a=_d.endX+1 -db;local _aa=d_a+cb-1
if _aa>=1 and
d_a<=ab then local aaa=math.max(1,d_a)local baa=math.min(ab,_aa)
local caa=baa-aaa+1
if caa>0 then local daa=string.rep(" ",caa)
self:blit(aaa,1,daa,string.rep(ca[self.getResolved("foreground")],caa),string.rep(ca[self.getResolved("background")],caa))end end end end end;local cc=self.getResolved("openDropdown")if cc then
self:renderDropdown(cc)end end
function da:renderDropdown(ab)local bb=self.getResolved("dropdownBackground")
local cb=self.getResolved("dropdownForeground")local db=ab.y
for _c,ac in ipairs(ab.items)do if type(ac)=="string"then ac={text=ac}
ab.items[_c]=ac end
local bc=self:getDropdownItemHeight(ac,ab.width-2)local cc=db;local dc=ac.text or ac.label or""
local _d=ac.content or ac.image;local ad=dc=="---"local bd=ca[ac.background or bb]
local cd=ca[ac.foreground or cb]local dd=string.rep(" ",ab.width)for row=0,bc-1 do
self:blit(ab.x,cc+row,dd,string.rep(cd,ab.width),string.rep(bd,ab.width))end
if ad then
local __a=string.rep("-",ab.width)
self:blit(ab.x,cc,__a,string.rep(ca[colors.gray],ab.width),string.rep(bd,ab.width))else
if _d~=nil then
self:drawContent(ab.x+1,cc,ab.width-2,bc,_d,{foreground=ac.foreground or cb,background=ac.background or bb})else
if#dc>ab.width-2 then dc=dc:sub(1,ab.width-2)end
self:textFg(ab.x+1,cc,dc,ac.foreground or cb)end end;db=db+bc end end
...
for ad,bd in ipairs(cc)do
if
type(bd)=="string"then bd={text=" "..bd.." "}cc[ad]=bd end
local cd=select(1,self:getContentSize(bd.content or bd.image,bd.text or""))
...
function da:showDropdown(ab,bb,cb)local db=bb.dropdown;if not db or#db==0 then return end;local _c=8
for cc,dc in
ipairs(db)do if type(dc)=="string"then dc={text=dc}db[cc]=dc end;local _d=dc.text or
dc.label or""local ad=
select(1,self:getContentSize(dc.content or dc.image,_d))+2
if ad>_c then _c=ad end end;local ac=0;for cc,dc in ipairs(db)do
ac=ac+self:getDropdownItemHeight(dc,_c-2)end
...
function da:handleDropdownClick(ab,bb,cb)local db=cb.y
for _c,ac in ipairs(cb.items)do
local bc=self:getDropdownItemHeight(ac,cb.width-2)
if bb>=db and bb<db+bc then local cc=ac;if type(cc)=="string"then cc={text=cc}
cb.items[_c]=cc end;if cc.text=="---"or cc.label=="---"or
cc.disabled then return true end
if cc.callback then
cc.callback(self,cc)elseif cc.onClick then cc.onClick(self,cc)end;self:hideDropdown()return true end;db=db+bc end;return false end;return da end
...
local cb=#bb+1
table.insert(bb,{id=cb,title=type(ab)=="table"and ab or
tostring(ab or("Item "..cb))})self.set("tabs",bb)if
...
local _c=self.getResolved("sidebarPosition")or"left"local ac={}local bc=1;local cc=0
for dc,_d in ipairs(ab)do
local ad,bd=self:getContentSize(_d.title)bd=math.max(1,bd)local cd=bc-db;local dd=0;local __a=0;if cd<1 then dd=1 -cd end;if
cd+bd-1 >bb then __a=(cd+bd-1)-bb end
if
cd+bd>1 and cd<=bb then local a_a=math.max(1,cd)local b_a=bd-dd-__a
table.insert(ac,{id=_d.id,title=_d.title,y1=a_a,y2=a_a+b_a-1,height=bd,displayHeight=b_a,actualY=bc,startClip=dd,endClip=__a})end;bc=bc+bd;cc=cc+bd end;return
...
do
ba.multiBlit(self,1,cc.y1 +dy,_c,1," ",da[bb],da[dc])end
ba.drawContent(self,2,cc.y1,_c-2,ad,cc.title,{sourceY=cc.startClip or 0,foreground=_d,background=dc,raw=true})end;if not self.getResolved("childrenSorted")then
self:sortChildren()end
if
not self.getResolved("childrenEventsSorted")then for bc in pairs(self._values.childrenEvents or{})do
self:sortChildrenEvents(bc)end end
...
local cb=#bb+1
table.insert(bb,{id=cb,title=type(ab)=="table"and ab or
tostring(ab or("Tab "..cb))})self.set("tabs",bb)if
not self.getResolved("activeTab")then self.set("activeTab",cb)end
self:updateTabVisibility()local db=self;local _c={}
setmetatable(_c,{__index=function(ac,bc)
if
...
if db then local ac=
self.getResolved("tabScrollOffset")or 0;local bc=1;local cc=0;local dc=cb
for _d,ad in ipairs(ab)do
local bd,cd=self:getContentSize(ad.title)bd=bd+2;cd=math.max(cb,cd)dc=math.max(dc,cd)
if bd>bb then bd=bb end;local dd=bc-ac;local __a=0;local a_a=0;if dd<1 then __a=1 -dd end;if dd+bd-1 >bb then a_a=
(dd+bd-1)-bb end
if dd+bd>1 and dd<=bb then
local b_a=math.max(1,dd)local c_a=bd-__a-a_a
table.insert(_c,{id=ad.id,title=ad.title,y1=1,y2=cd,x1=b_a,x2=b_a+c_a-1,width=bd,height=cd,displayWidth=c_a,actualX=bc,startClip=__a,endClip=a_a})end;bc=bc+bd end;cc=bc-1;return
{headerHeight=dc,lines=1,positions=_c,totalWidth=cc,scrollOffset=ac,maxScroll=math.max(0,cc-bb)}else local ac={}local bc=1
local cc={items={},height=cb}table.insert(ac,cc)
for _d,ad in ipairs(ab)do
local bd,cd=self:getContentSize(ad.title)bd=bd+2;cd=math.max(cb,cd)if bd>bb then bd=bb end;if bc+bd-1 >bb then
cc={items={},height=cb}table.insert(ac,cc)bc=1 end
cc.height=math.max(cc.height,cd)
table.insert(cc.items,{id=ad.id,title=ad.title,x1=bc,x2=bc+bd-1,width=bd,height=cd})bc=bc+bd end;local dc=0
for _d,ad in ipairs(ac)do local bd=dc+1;local cd=bd+ad.height-1;dc=cd;for dd,__a in
ipairs(ad.items)do __a.line=_d;__a.y1=bd;__a.y2=cd;__a.displayHeight=ad.height
table.insert(_c,__a)end end;return{headerHeight=math.max(cb,dc),lines=#ac,positions=_c}end end
function _b:mouse_click(ab,bb,cb)
if not ba.mouse_click(self,ab,bb,cb)then return false end;local db,_c=ba.getRelativePosition(self,bb,cb)
local ac=self:_getHeaderMetrics()
if _c<=ac.headerHeight then if#ac.positions==0 then return true end
for bc,cc in
ipairs(ac.positions)do if
_c>=cc.y1 and _c<=cc.y2 and db>=cc.x1 and db<=cc.x2 then self:setActiveTab(cc.id)
self.set("focusedChild",nil)return true end end;return true end;return ca.mouse_click(self,ab,bb,cb)end
...
local db,_c=ba.getRelativePosition(self,bb,cb)local ac=self:_getHeaderMetrics().headerHeight
if
self.getResolved("scrollableTab")and _c<=ac then self:scrollTabs(ab)return true end;return ca.mouse_scroll(self,ab,bb,cb)end;return false end
...
ba.multiBlit(self,cc.x1,cc.y1,
cc.displayWidth or(cc.x2 -cc.x1 +1),cc.displayHeight or cc.height or 1," ",da[bb],da[dc])
ba.drawContent(self,cc.x1 +1,cc.y1,math.max(0,(cc.displayWidth or cc.width)-2),
cc.displayHeight or cc.height or 1,cc.title,{sourceX=cc.startClip or 0,foreground=_d,background=dc,raw=true})end;if not self.getResolved("childrenSorted")then
self:sortChildren()end
if
not self.getResolved("childrenEventsSorted")then for bc in pairs(self._values.childrenEvents or{})do
self:sortChildrenEvents(bc)end end
...
project["elements/VisualElement.lua"] = function(...) local db=require("elementManager")
local _c=db.getElement("BaseElement")local ac=require("libraries/colorHex")
local bc=setmetatable({},_c)bc.__index=bc
bc.defineProperty(bc,"x",{default=1,type="number",canTriggerRender=true,setter=function(b_a,c_a)
if b_a.parent then
b_a.parent.set("childrenSorted",false)b_a.parent.set("childrenEventsSorted",false)end;return c_a end})
bc.defineProperty(bc,"y",{default=1,type="number",canTriggerRender=true,setter=function(b_a,c_a)
if b_a.parent then
b_a.parent.set("childrenSorted",false)b_a.parent.set("childrenEventsSorted",false)end;return c_a end})
bc.defineProperty(bc,"z",{default=1,type="number",canTriggerRender=true,setter=function(b_a,c_a)
if b_a.parent then b_a.parent:sortChildren()end;return c_a end})
bc.defineProperty(bc,"constraints",{default={},type="table"})
bc.defineProperty(bc,"width",{default=1,type="number",canTriggerRender=true,setter=function(b_a,c_a)
if b_a.parent then
b_a.parent.set("childrenSorted",false)b_a.parent.set("childrenEventsSorted",false)end;return c_a end})
bc.defineProperty(bc,"height",{default=1,type="number",canTriggerRender=true,setter=function(b_a,c_a)
if b_a.parent then
b_a.parent.set("childrenSorted",false)b_a.parent.set("childrenEventsSorted",false)end;return c_a end})
bc.defineProperty(bc,"background",{default=colors.black,type="color",canTriggerRender=true})
bc.defineProperty(bc,"foreground",{default=colors.white,type="color",canTriggerRender=true})
bc.defineProperty(bc,"backgroundEnabled",{default=true,type="boolean",canTriggerRender=true})
bc.defineProperty(bc,"borderTop",{default=false,type="boolean",canTriggerRender=true})
bc.defineProperty(bc,"borderBottom",{default=false,type="boolean",canTriggerRender=true})
bc.defineProperty(bc,"borderLeft",{default=false,type="boolean",canTriggerRender=true})
bc.defineProperty(bc,"borderRight",{default=false,type="boolean",canTriggerRender=true})
bc.defineProperty(bc,"borderColor",{default=colors.white,type="color",canTriggerRender=true})
bc.defineProperty(bc,"visible",{default=true,type="boolean",canTriggerRender=true,setter=function(b_a,c_a)
if(b_a.parent~=nil)then
b_a.parent.set("childrenSorted",false)b_a.parent.set("childrenEventsSorted",false)end;if(c_a==false)then b_a:unsetState("clicked")end
return c_a end})
bc.defineProperty(bc,"ignoreOffset",{default=false,type="boolean"})
bc.defineProperty(bc,"layoutConfig",{default={},type="table"})bc.combineProperties(bc,"position","x","y")
bc.combineProperties(bc,"size","width","height")
bc.combineProperties(bc,"color","foreground","background")bc.defineEvent(bc,"focus")
bc.defineEvent(bc,"blur")
bc.registerEventCallback(bc,"Click","mouse_click","mouse_up")
bc.registerEventCallback(bc,"DoubleClick","mouse_double_click","mouse_click")
bc.registerEventCallback(bc,"ClickUp","mouse_up","mouse_click")
bc.registerEventCallback(bc,"Drag","mouse_drag","mouse_click","mouse_up")
bc.registerEventCallback(bc,"Scroll","mouse_scroll")
bc.registerEventCallback(bc,"Enter","mouse_enter","mouse_move")
bc.registerEventCallback(bc,"Leave","mouse_leave","mouse_move")bc.registerEventCallback(bc,"Focus","focus","blur")
bc.registerEventCallback(bc,"Blur","blur","focus")bc.registerEventCallback(bc,"Key","key","key_up")
bc.registerEventCallback(bc,"Char","char")bc.registerEventCallback(bc,"KeyUp","key_up","key")
local cc,dc=math.max,math.min;local function _d(b_a)
return type(b_a)=="table"and type(b_a[1])=="string"end;local function ad(b_a)return
type(b_a)=="table"and _d(b_a[1])end
local function bd(b_a)local c_a=b_a
while



type(c_a)=="table"and#c_a==1 and type(c_a[1])=="table"and type(c_a[1][1])=="table"and type(c_a[1][1][1])=="string"do c_a=c_a[1]end;if ad(c_a)then return c_a end;return nil end
local function cd(b_a,c_a,d_a,_aa)if type(b_a)~="string"or#b_a==0 then return b_a end
b_a=b_a:gsub(" ",_aa)b_a=b_a:gsub("Q",c_a)b_a=b_a:gsub("B",d_a)return b_a end
local function dd(b_a)if type(b_a)~="table"then return nil end
if
type(b_a.getFrame)=="function"then local d_a=b_a:getFrame(b_a.currentFrame)return bd(d_a)end;local c_a=b_a.bimg or b_a;if type(c_a)~="table"then return nil end;if
ad(c_a)then return c_a end
if type(c_a[1])=="table"then return bd(
c_a[b_a.currentFrame or 1]or c_a[1])end;return nil end
local function __a(b_a)if type(b_a)=="string"then return#b_a,1,"text"end
local c_a=dd(b_a)if not c_a then return 0,0,nil end;local d_a,_aa=0,0;for aaa,baa in pairs(c_a)do
if type(baa)=="table"and
type(baa[1])=="string"then
d_a=math.max(d_a,#baa[1])_aa=math.max(_aa,aaa)end end
return d_a,_aa,"image",c_a end;function bc.new()local b_a=setmetatable({},bc):__init()
b_a.class=bc;return b_a end
function bc:init(b_a,c_a)
_c.init(self,b_a,c_a)self.set("type","VisualElement")
...
function bc:setConstraint(b_a,c_a,d_a,_aa)local aaa=self.get("constraints")if aaa[b_a]then
self:_removeConstraintObservers(b_a,aaa[b_a])end
aaa[b_a]={element=c_a,property=d_a,offset=_aa or 0}self.set("constraints",aaa)
self:_addConstraintObservers(b_a,aaa[b_a])self._constraintsDirty=true;self:updateRender()return self end
function bc:setLayoutConfigProperty(b_a,c_a)local d_a=self.getResolved("layoutConfig")
d_a[b_a]=c_a;self.set("layoutConfig",d_a)return self end;function bc:getLayoutConfigProperty(b_a)local c_a=self.getResolved("layoutConfig")
return c_a[b_a]end
function bc:resolveAllConstraints()if not
self._constraintsDirty then return self end
local b_a=self.getResolved("constraints")if not b_a or not next(b_a)then return self end
local c_a={"width","height","left","right","top","bottom","x","y","centerX","centerY"}for d_a,_aa in ipairs(c_a)do
if b_a[_aa]then
local aaa=self:_resolveConstraint(_aa,b_a[_aa])self:_applyConstraintValue(_aa,aaa,b_a)end end
self._constraintsDirty=false;return self end
function bc:_applyConstraintValue(b_a,c_a,d_a)
if b_a=="x"or b_a=="left"then self.set("x",c_a)elseif b_a=="y"or
b_a=="top"then self.set("y",c_a)elseif b_a=="right"then
if d_a.left then
local _aa=self:_resolveConstraint("left",d_a.left)local aaa=c_a-_aa+1;self.set("width",aaa)
self.set("x",_aa)else local _aa=self.getResolved("width")
self.set("x",c_a-_aa+1)end elseif b_a=="bottom"then
if d_a.top then
local _aa=self:_resolveConstraint("top",d_a.top)local aaa=c_a-_aa+1;self.set("height",aaa)
self.set("y",_aa)else local _aa=self.getResolved("height")
self.set("y",c_a-_aa+1)end elseif b_a=="centerX"then local _aa=self.getResolved("width")self.set("x",c_a-
math.floor(_aa/2))elseif b_a=="centerY"then
local _aa=self.getResolved("height")self.set("y",c_a-math.floor(_aa/2))elseif
b_a=="width"then self.set("width",c_a)elseif b_a=="height"then
self.set("height",c_a)end end
function bc:_addConstraintObservers(b_a,c_a)local d_a=c_a.element;local _aa=c_a.property;if d_a=="parent"then
d_a=self.parent end;if not d_a then return end
local aaa=function()self._constraintsDirty=true
self:resolveAllConstraints()self:updateRender()end
if not self._constraintObserverCallbacks then self._constraintObserverCallbacks={}end;if not self._constraintObserverCallbacks[b_a]then
self._constraintObserverCallbacks[b_a]={}end;local baa={}
if
_aa=="left"or _aa=="x"then baa={"x"}elseif _aa=="right"then baa={"x","width"}elseif _aa=="top"or _aa=="y"then
baa={"y"}elseif _aa=="bottom"then baa={"y","height"}elseif _aa=="centerX"then baa={"x","width"}elseif _aa==
"centerY"then baa={"y","height"}elseif _aa=="width"then baa={"width"}elseif _aa=="height"then
baa={"height"}end;for caa,daa in ipairs(baa)do d_a:observe(daa,aaa)
table.insert(self._constraintObserverCallbacks[b_a],{element=d_a,property=daa,callback=aaa})end end
function bc:_removeConstraintObservers(b_a,c_a)
if not self._constraintObserverCallbacks or not
self._constraintObserverCallbacks[b_a]then return end;for d_a,_aa in ipairs(self._constraintObserverCallbacks[b_a])do
_aa.element:removeObserver(_aa.property,_aa.callback)end;self._constraintObserverCallbacks[b_a]=
nil end
function bc:_removeAllConstraintObservers()
if not self._constraintObserverCallbacks then return end
for b_a,c_a in pairs(self._constraintObserverCallbacks)do for d_a,_aa in ipairs(c_a)do
_aa.element:removeObserver(_aa.property,_aa.callback)end end;self._constraintObserverCallbacks=nil end
function bc:removeConstraint(b_a)local c_a=self.getResolved("constraints")
c_a[b_a]=nil;self.set("constraints",c_a)
self:updateConstraints()return self end
function bc:updateConstraints()local b_a=self.getResolved("constraints")
for c_a,d_a in pairs(b_a)do
local _aa=self:_resolveConstraint(c_a,d_a)
if c_a=="x"or c_a=="left"then self.set("x",_aa)elseif
c_a=="y"or c_a=="top"then self.set("y",_aa)elseif c_a=="right"then
local aaa=self.getResolved("width")self.set("x",_aa-aaa+1)elseif c_a=="bottom"then
local aaa=self.getResolved("height")self.set("y",_aa-aaa+1)elseif c_a=="centerX"then
local aaa=self.getResolved("width")self.set("x",_aa-math.floor(aaa/2))elseif
c_a=="centerY"then local aaa=self.getResolved("height")self.set("y",_aa-
math.floor(aaa/2))elseif c_a=="width"then
self.set("width",_aa)elseif c_a=="height"then self.set("height",_aa)end end end
function bc:_resolveConstraint(b_a,c_a)if type(b_a)~="string"then return 1 end;local d_a=c_a.element
local _aa=c_a.property;local aaa=c_a.offset;if d_a=="parent"then d_a=self.parent end;if not d_a then return
self.getResolved(b_a)or 1 end;local baa
if _aa=="left"or
_aa=="x"then baa=d_a.get("x")elseif _aa=="right"then baa=
d_a.get("x")+d_a.get("width")-1 elseif _aa=="top"or _aa=="y"then
baa=d_a.get("y")elseif _aa=="bottom"then
baa=d_a.get("y")+d_a.get("height")-1 elseif _aa=="centerX"then
baa=d_a.get("x")+math.floor(d_a.get("width")/2)elseif _aa=="centerY"then baa=d_a.get("y")+
math.floor(d_a.get("height")/2)elseif _aa=="width"then
baa=d_a.get("width")elseif _aa=="height"then baa=d_a.get("height")end
if type(aaa)=="number"then if aaa>-1 and aaa<1 and aaa~=0 then return
math.floor(baa*aaa)else return baa+aaa end end;return baa end;function bc:alignRight(b_a,c_a)c_a=c_a or 0
return self:setConstraint("right",b_a,"right",c_a)end;function bc:alignLeft(b_a,c_a)c_a=c_a or 0;return
self:setConstraint("left",b_a,"left",c_a)end
function bc:alignTop(b_a,c_a)c_a=
c_a or 0;return self:setConstraint("top",b_a,"top",c_a)end;function bc:alignBottom(b_a,c_a)c_a=c_a or 0
return self:setConstraint("bottom",b_a,"bottom",c_a)end
function bc:centerHorizontal(b_a,c_a)c_a=c_a or 0;return
self:setConstraint("centerX",b_a,"centerX",c_a)end;function bc:centerVertical(b_a,c_a)c_a=c_a or 0
return self:setConstraint("centerY",b_a,"centerY",c_a)end;function bc:centerIn(b_a)return
self:centerHorizontal(b_a):centerVertical(b_a)end;function bc:rightOf(b_a,c_a)c_a=
c_a or 0
return self:setConstraint("left",b_a,"right",c_a)end
function bc:leftOf(b_a,c_a)c_a=c_a or 0;return self:setConstraint("right",b_a,"left",
-c_a)end;function bc:below(b_a,c_a)c_a=c_a or 0
return self:setConstraint("top",b_a,"bottom",c_a)end
function bc:above(b_a,c_a)c_a=c_a or 0;return self:setConstraint("bottom",b_a,"top",
-c_a)end;function bc:stretchWidth(b_a,c_a)c_a=c_a or 0
return self:setConstraint("left",b_a,"left",c_a):setConstraint("right",b_a,"right",
-c_a)end;function bc:stretchHeight(b_a,c_a)c_a=
c_a or 0
return self:setConstraint("top",b_a,"top",c_a):setConstraint("bottom",b_a,"bottom",
-c_a)end;function bc:stretch(b_a,c_a)return
self:stretchWidth(b_a,c_a):stretchHeight(b_a,c_a)end
function bc:widthPercent(b_a,c_a)return self:setConstraint("width",b_a,"width",
c_a/100)end;function bc:heightPercent(b_a,c_a)
return self:setConstraint("height",b_a,"height",c_a/100)end;function bc:matchWidth(b_a,c_a)c_a=c_a or 0;return
self:setConstraint("width",b_a,"width",c_a)end;function bc:matchHeight(b_a,c_a)c_a=
c_a or 0
return self:setConstraint("height",b_a,"height",c_a)end;function bc:fillParent(b_a)return
self:stretch("parent",b_a)end;function bc:fillWidth(b_a)return
self:stretchWidth("parent",b_a)end;function bc:fillHeight(b_a)return
self:stretchHeight("parent",b_a)end;function bc:center()return
self:centerIn("parent")end;function bc:toRight(b_a)return
self:alignRight("parent",- (b_a or 0))end
function bc:toLeft(b_a)return self:alignLeft("parent",
b_a or 0)end
function bc:toTop(b_a)return self:alignTop("parent",b_a or 0)end;function bc:toBottom(b_a)
return self:alignBottom("parent",- (b_a or 0))end
function bc:multiBlit(b_a,c_a,d_a,_aa,aaa,baa,caa)
local daa,_ba=self:calculatePosition()b_a=b_a+daa-1;c_a=c_a+_ba-1
self.parent:multiBlit(b_a,c_a,d_a,_aa,aaa,baa,caa)end
function bc:textFg(b_a,c_a,d_a,_aa)local aaa,baa=self:calculatePosition()b_a=b_a+aaa-1;c_a=
c_a+baa-1;self.parent:textFg(b_a,c_a,d_a,_aa)end
function bc:setContent(b_a)self._content=b_a;self:updateRender()return self end;function bc:getContent()return self._content end;function bc:setImage(b_a)return
self:setContent(b_a)end;function bc:getImage()
return self:getContent()end;function bc:getContentSize(b_a,c_a)local d_a=b_a;if d_a==nil then d_a=c_a end;return
__a(d_a)end
function bc:drawContent(b_a,c_a,d_a,_aa,aaa,baa)
baa=baa or{}local caa=aaa;if caa==nil then caa=baa.fallbackText end
if caa==nil then return false end;local daa,_ba,aba,bba=__a(caa)
if aba==nil or daa<=0 or _ba<=0 then return false end;local cba=d_a or daa;local dba=_aa or _ba;local _ca=baa.sourceX or 0
local aca=baa.sourceY or 0
local bca=ac[baa.foreground or self.getResolved("foreground")]
local cca=ac[baa.background or self.getResolved("background")]
if aba=="text"then local _da=caa;if _ca>0 then _da=_da:sub(_ca+1)end;if cba and
#_da>cba then _da=_da:sub(1,cba)end
if#_da>0 and
(dba==nil or dba>0)then
if baa.raw then
bc.textFg(self,b_a,c_a,_da,baa.foreground or self.getResolved("foreground"))else
self:textFg(b_a,c_a,_da,baa.foreground or self.getResolved("foreground"))end;return true end;return false end;local dca=math.min(_ba,aca+dba)
for row=aca+1,dca do local _da=bba[row]
if
type(_da)=="table"and type(_da[1])=="table"and
type(_da[1][1])=="string"then _da=_da[1]end
if _d(_da)then local ada=_da[1]or""local bda=cd(_da[2]or"",bca,cca,bca)local cda=cd(
_da[3]or"",bca,cca,cca)
if _ca>0 then ada=ada:sub(_ca+1)bda=bda:sub(
_ca+1)cda=cda:sub(_ca+1)end
if cba then ada=ada:sub(1,cba)bda=bda:sub(1,cba)cda=cda:sub(1,cba)end
if type(bda)=="string"and type(cda)=="string"then local dda=math.min(#ada,#bda,
#cda)if dda>0 then ada=ada:sub(1,dda)
bda=bda:sub(1,dda)cda=cda:sub(1,dda)end end
if
type(bda)=="string"and type(cda)=="string"and#ada>0 then if baa.raw then
bc.blit(self,b_a,c_a+ (row-aca-1),ada,bda,cda)else
self:blit(b_a,c_a+ (row-aca-1),ada,bda,cda)end end end end;return true end
function bc:textBg(b_a,c_a,d_a,_aa)local aaa,baa=self:calculatePosition()b_a=b_a+aaa-1;c_a=
c_a+baa-1;self.parent:textBg(b_a,c_a,d_a,_aa)end
function bc:drawText(b_a,c_a,d_a)local _aa,aaa=self:calculatePosition()b_a=b_a+_aa-1;c_a=
c_a+aaa-1;self.parent:drawText(b_a,c_a,d_a)end
function bc:drawFg(b_a,c_a,d_a)local _aa,aaa=self:calculatePosition()b_a=b_a+_aa-1;c_a=
c_a+aaa-1;self.parent:drawFg(b_a,c_a,d_a)end
function bc:drawBg(b_a,c_a,d_a)local _aa,aaa=self:calculatePosition()b_a=b_a+_aa-1;c_a=
c_a+aaa-1;self.parent:drawBg(b_a,c_a,d_a)end
function bc:blit(b_a,c_a,d_a,_aa,aaa)local baa,caa=self:calculatePosition()b_a=b_a+baa-1;c_a=
c_a+caa-1;self.parent:blit(b_a,c_a,d_a,_aa,aaa)end
function bc:isInBounds(b_a,c_a)
local d_a,_aa=self.getResolved("x"),self.getResolved("y")
local aaa,baa=self.getResolved("width"),self.getResolved("height")
if(self.getResolved("ignoreOffset"))then if(self.parent)then b_a=b_a-
self.parent.get("offsetX")
c_a=c_a-self.parent.get("offsetY")end end
return b_a>=d_a and b_a<=d_a+aaa-1 and c_a>=_aa and c_a<=
_aa+baa-1 end;local a_a=0.4
function bc:mouse_click(b_a,c_a,d_a)
if self:isInBounds(c_a,d_a)then
self:setState("clicked")local _aa,aaa=self:getRelativePosition(c_a,d_a)
local baa=os.clock()local caa=self._lastClick
if caa and(baa-caa.time)<=a_a and
caa.button==b_a then self._lastClick=nil
self:fireEvent("mouse_double_click",b_a,_aa,aaa)else self._lastClick={time=baa,button=b_a}
self:fireEvent("mouse_click",b_a,_aa,aaa)end;return true end;return false end
function bc:mouse_up(b_a,c_a,d_a)
if self:isInBounds(c_a,d_a)then self:unsetState("clicked")
self:unsetState("dragging")
self:fireEvent("mouse_up",b_a,self:getRelativePosition(c_a,d_a))return true end;return false end
function bc:mouse_release(b_a,c_a,d_a)
self:fireEvent("mouse_release",b_a,self:getRelativePosition(c_a,d_a))self:unsetState("clicked")
self:unsetState("dragging")end
function bc:mouse_move(b_a,c_a,d_a)if(c_a==nil)or(d_a==nil)then return false end
local _aa=self:hasState("hover")
if(self:isInBounds(c_a,d_a))then if(not _aa)then self:setState("hover")
self:fireEvent("mouse_enter",self:getRelativePosition(c_a,d_a))end;return true else if(_aa)then
self:unsetState("hover")
self:fireEvent("mouse_leave",self:getRelativePosition(c_a,d_a))end end;return false end
function bc:mouse_scroll(b_a,c_a,d_a)if(self:isInBounds(c_a,d_a))then
self:fireEvent("mouse_scroll",b_a,self:getRelativePosition(c_a,d_a))return true end;return false end
function bc:mouse_drag(b_a,c_a,d_a)if(self:hasState("clicked"))then
self:fireEvent("mouse_drag",b_a,self:getRelativePosition(c_a,d_a))return true end;return false end
function bc:setFocused(b_a,c_a)local d_a=self:hasState("focused")
if b_a==d_a then return self end
if b_a then self:setState("focused")self:focus()
if
not c_a and self.parent then self.parent:setFocusedChild(self)end else self:unsetState("focused")self:blur()
if
not c_a and self.parent then self.parent:setFocusedChild(nil)end end;return self end;function bc:focus()self:fireEvent("focus")end;function bc:blur()
self:fireEvent("blur")
self:setCursor(1,1,false,self.getResolved("foreground"))end;function bc:isFocused()return
self:hasState("focused")end
function bc:addBorder(b_a,c_a)local d_a=nil;local _aa=nil
if type(b_a)==
"table"and
(b_a.color or b_a.top~=nil or b_a.left~=nil)then d_a=b_a.color;_aa=b_a else d_a=b_a;_aa=c_a end
if _aa then
if _aa.top~=nil then self.set("borderTop",_aa.top)end
if _aa.bottom~=nil then self.set("borderBottom",_aa.bottom)end
if _aa.left~=nil then self.set("borderLeft",_aa.left)end
if _aa.right~=nil then self.set("borderRight",_aa.right)end else self.set("borderTop",true)
self.set("borderBottom",true)self.set("borderLeft",true)
self.set("borderRight",true)end;if d_a then self.set("borderColor",d_a)end;return self end
function bc:removeBorder()self.set("borderTop",false)
self.set("borderBottom",false)self.set("borderLeft",false)
self.set("borderRight",false)return self end;function bc:key(b_a,c_a)
if(self:hasState("focused"))then self:fireEvent("key",b_a,c_a)end end
function bc:key_up(b_a)if
(self:hasState("focused"))then self:fireEvent("key_up",b_a)end end;function bc:char(b_a)
if(self:hasState("focused"))then self:fireEvent("char",b_a)end end
function bc:calculatePosition()
self:resolveAllConstraints()local b_a,c_a=self.getResolved("x"),self.getResolved("y")
if not
self.getResolved("ignoreOffset")then
if self.parent~=nil then
local d_a,_aa=self.parent.get("offsetX"),self.parent.get("offsetY")b_a=b_a-d_a;c_a=c_a-_aa end end;return b_a,c_a end
function bc:getAbsolutePosition(b_a,c_a)
local d_a,_aa=self.getResolved("x"),self.getResolved("y")if(b_a~=nil)then d_a=d_a+b_a-1 end
if(c_a~=nil)then _aa=_aa+c_a-1 end;local aaa=self.parent
while aaa do local baa,caa=aaa.get("x"),aaa.get("y")
d_a=d_a+baa-1;_aa=_aa+caa-1;aaa=aaa.parent end;return d_a,_aa end
function bc:getRelativePosition(b_a,c_a)if(b_a==nil)or(c_a==nil)then
b_a,c_a=self.getResolved("x"),self.getResolved("y")end;local d_a,_aa=1,1;if self.parent then
d_a,_aa=self.parent:getRelativePosition()end
local aaa,baa=self.getResolved("x"),self.getResolved("y")
return b_a- (aaa-1)- (d_a-1),c_a- (baa-1)- (_aa-1)end
function bc:setCursor(b_a,c_a,d_a,_aa)
if self.parent then local aaa,baa=self:calculatePosition()
if


(b_a+aaa-1 <1)or(b_a+aaa-1 >self.parent.get("width"))or(c_a+baa-1 <1)or(c_a+baa-1 >
self.parent.get("height"))then
return self.parent:setCursor(b_a+aaa-1,c_a+baa-1,false)end
return self.parent:setCursor(b_a+aaa-1,c_a+baa-1,d_a,_aa)end;return self end
function bc:prioritize()
if(self.parent)then local b_a=self.parent;b_a:removeChild(self)
b_a:addChild(self)self:updateRender()end;return self end
function bc:render()
if(not self.getResolved("backgroundEnabled"))then return end
local b_a,c_a=self.getResolved("width"),self.getResolved("height")local d_a=ac[self.getResolved("foreground")]
local _aa=ac[self.getResolved("background")]
local aaa,baa,caa,daa=self.getResolved("borderTop"),self.getResolved("borderBottom"),self.getResolved("borderLeft"),self.getResolved("borderRight")self:multiBlit(1,1,b_a,c_a," ",d_a,_aa)
if(aaa or baa or caa or
daa)then local _ba=self.getResolved("borderColor")or
self.getResolved("foreground")local aba=ac[_ba]or
d_a;if aaa then
self:textFg(1,1,("\131"):rep(b_a),_ba)end;if baa then
self:multiBlit(1,c_a,b_a,1,"\143",_aa,aba)end;if caa then
self:multiBlit(1,1,1,c_a,"\149",aba,_aa)end;if daa then
self:multiBlit(b_a,1,1,c_a,"\149",_aa,aba)end;if aaa and caa then
self:blit(1,1,"\151",aba,_aa)end;if aaa and daa then
self:blit(b_a,1,"\148",_aa,aba)end;if baa and caa then
self:blit(1,c_a,"\138",_aa,aba)end;if baa and daa then
self:blit(b_a,c_a,"\133",_aa,aba)end end end;function bc:postRender()end
function bc:destroy()
self:_removeAllConstraintObservers()self.set("visible",false)_c.destroy(self)end;return bc end
...
if d==self then return c end end;return nil end
function b:setText(c)self._data.text=c;self._data.content=nil
self._data.image=nil;self._parent:updateRender()return self end;function b:getText()return self._data.text end;function b:setContent(c)
self._data.content=c;self._data.image=c;self._parent:updateRender()
return self end
function b:getContent()return
self._data.content or self._data.image end;function b:setImage(c)return self:setContent(c)end;function b:getImage()return
self:getContent()end
function b:moveUp(c)
local d=self._parent:getItems()local _a=self:_findIndex()if not _a then return self end;c=c or 1
local aa=math.max(1,_a-c)if _a~=aa then table.remove(d,_a)table.insert(d,aa,self)
...
local function bd(b_a,c_a)local d_a={}c_a=math.max(1,c_a)while#b_a>0 do local _aa=b_a:sub(1,c_a)
table.insert(d_a,_aa)b_a=b_a:sub(c_a+1)end;if
#d_a==0 then d_a[1]=""end;return d_a end
local function cd(b_a)local c_a={}local d_a=math.max(1,b_a.get("width")-1)
for _aa,aaa in
ipairs(_b._logs)do
local baa=aaa.parts or{{type="text",text=aaa.message or""}}
for caa,daa in ipairs(baa)do
if daa.type=="bimg"and daa.bimg~=nil then local _ba,aba=b_a:getContentSize(daa.bimg,
daa.text or"")
table.insert(c_a,{type="bimg",content=daa.bimg,height=math.max(1,aba),level=aaa.level})else local _ba=bd(daa.text or"",d_a)for aba,bba in ipairs(_ba)do
table.insert(c_a,{type="text",text=bba,height=1,level=aaa.level})end end end end;return c_a end
local dd=#cd(dc._debugFrame)-dc.get("height")dc._debugFrame._scrollOffset=math.max(0,dd)
local __a=dc._debugFrame.render
dc._debugFrame.render=function(b_a)__a(b_a)b_a._processedLogs=cd(b_a)local c_a=math.max(0,
b_a.get("height")-4)local d_a=0
for caa,daa in
ipairs(b_a._processedLogs)do d_a=d_a+ (daa.height or 1)end;local _aa=math.max(0,d_a-c_a)
b_a._scrollOffset=math.min(b_a._scrollOffset,_aa)local aaa=1;local baa=b_a._scrollOffset
for caa,daa in ipairs(b_a._processedLogs)do
local _ba=daa.height or 1
if baa>=_ba then baa=baa-_ba else local aba=baa
local bba=math.min(_ba-aba,c_a-aaa+1)
if bba>0 then
local cba=

daa.level==_b.LEVEL.ERROR and colors.red or daa.level==
_b.LEVEL.WARN and colors.yellow or daa.level==_b.LEVEL.DEBUG and colors.lightGray or colors.white
if daa.type=="bimg"and daa.content~=nil then
b_a:drawContent(2,aaa,_d,bba,daa.content,{sourceY=aba,foreground=cba,background=b_a.get("background"),raw=true})else b_a:textFg(2,aaa,daa.text or"",cba)end;aaa=aaa+bba;if aaa>c_a then break end end;baa=0 end end end;local a_a=dc._debugFrame.dispatchEvent
...
if ad[__a]then ad=ad[__a]bd=true;break end end;if not bd then end end;return ad end
...
for _aa,aaa in
pairs(dd.default)do if type(aaa)~="table"then cd[_aa]=aaa end end end;break end end;if not b_a then end end;local __a=_c(dc,_d)if __a then
